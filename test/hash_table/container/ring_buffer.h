...
public:
    ...
    uint32_t DoEnqueueOne(T** obj, RB_Property* property)
    {
        uint32_t prod_head, prod_next;
        uint32_t free_entries;
        uint32_t n = 1;
        ...
        property->free_space = free_entries - n;
        property->prod_head = prod_head;
        property->prod_next = prod_next;
        return n;
    }

    void UpdateProdTail(RB_Property* property) 
    {
        UpdateTail(&prod_, property->prod_head, property->prod_next, single_producer_);
    }
