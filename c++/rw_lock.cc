...
static __thread int g_core_num = 0;

std::map<uint64_t, std::string> g_map;

uint64_t g_cnt = 0;

bool test_cnt(uint64_t n)
{
   return n >= 10000 * 1000 * 5; 
}
...
void* load(void* arg)
{
    ...
    while (g_running) {
        ...
        if(test_cnt(__sync_add_and_fetch(&g_cnt, 1))) {
              printf("n=%lu, n_add=%lu\n", n, n_add);
              break;
        } 
